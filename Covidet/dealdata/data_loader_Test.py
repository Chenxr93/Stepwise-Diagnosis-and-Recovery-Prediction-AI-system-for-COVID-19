...
def crop(img):
    img = np.pad(img,((0,0),(0,0),(0,0)),'constant',constant_values=0)
    rand1 = np.random.randint(6,13) / 100.
    rand2 = np.random.randint(87,94)/ 100.
    rand3 = np.random.randint(6,13) / 100.
    rand4 = np.random.randint(87, 94) / 100.
    w,h,c = img.shape
...
def addFlip(img):
    ...
    if len(img.shape) == 4:
        ...
        if r>7:
            img = img[:,::-1,::-1]
    return img
def jumpGetALL(dirlist,total,b,numSeries,beginNum):
    return jumpGet(dirlist, total, b, numSeries,beginNum)
def jumpGet(dirlist,total,b,numSeries,beginNum):
    retlist = []
...
class DataBowl(Dataset):
    def __init__(self, phase='train', policy = None,wh = (256,256
                 ...
                 path_train = '/data/chenxiangru/covidData/TrainCov4-26',
                 path_val = '/data/chenxiangru/covidData/clinicalTest_deal_withname',
                 three=1, p1=1):
        ...
        self.three = three #1代笔三分类 中是否肺炎，2 代表三分类中是否新冠
        self.p1 = p1
        self.stdmean = {
    ...
    def resize(self,image,name = None,needtrans = False):
        ...
        image =  image.transpose((2,1,0))
        return image
    def parse_policy(self, policy_emb, augmentation_transforms=None):
        self.policy = []
        #print(policy_emb)
        num_xform = 16 #augmentation_transforms.NUM_HP_TRANSFORM
        xform_names = ['name'] #augmentation_transforms.HP_TRANSFORM_NAMES
    ...
    def __getitem__(self, item):
        ...
        images = []
        if self.p1 ==1:
            label = int(self.imageList[item].split('/')[-1].split('_')[0])
        ...
        #label = int(self.imageList[item].split('/')[-1].split(',')[-1][0])
        #label = int(self.imageList[item].split('/')[-1].split(',')[-1].split('.')[0])
        #label = int(self.imageList[item].split('/')[-1].split('_')[1])#.split('.')[0])
        ...
        if np.random.randint(0, 10) > 3:
            rotatenum = np.random.randint(0,360)
        else:
            rotatenum = 0.
        autocontrastrand=  np.random.randint(0,20)
        for i in range(len(fs)):
            ...
            # img = ImageOps.autocontrast(img,autocontrastrand)
            img = np.array(img)
            # print(img.shape)
            img = crop(img)
        ...
        if length>numSeries:
            b = length // numSeries
            groups = length // numSeries+ length - (length//numSeries)*numSeries
            print(length,groups)
            for g in range(groups):
                imgblock = jumpGetALL(images,length,b,numSeries,g)
                # print(imgblock.shape)
                imgblocks.append(imgblock)
        else:
            b = 1
            imgblock = jumpGetALL(images, length, b, numSeries, 0)
            imgblocks.append(imgblock)
        if self.policy is None:
            for i in range(len(imgblocks)):
                imgblocks[i] = torch.Tensor((np.ascontiguousarray(imgblocks[i]/255.)))
            # self.image = torch.Tensor((np.ascontiguousarray((imgblock/255. - self.meanmap)/self.stdmap)))
            return imgblocks, torch.tensor(label).float(), self.imageList[item].split('/')[-1]
        else:
