...
def changeColorSpace(enhanced_img):
    ...
    return enhanced_img
def findDepestFolder(path,finallist):
    dir = os.listdir(path)
    if len(dir) == 0:
        return
    dirs = []
...
class DataBowl(Dataset):
    def __init__(self, phase='train', policy = None):
        assert (phase == 'train' or phase == 'val' or phase == 'test')
        self.phase = phase
        self.policy = policy
        self.path_positive = '/sharedata/gaozebin/FightNCP/positive'
        self.path_negtive = '/sharedata/gaozebin/FightNCP/negtive'
        self.stdmean = {
        ...
        self.imageAll = []
        if phase == 'test':
            self.test = [i for i in os.listdir(self.testpath)]
            self.test = [os.path.join(self.testpath, i) for i in sorted(self.test)]
            self.rotate = False
            ...
            for fd in self.image_name_positive:
                ...
                for folder in fdlist:
                    if len(os.listdir(folder)) == 0:
                        continue
                    self.imageAll.append([[os.path.join(folder,i) for i in sorted(os.listdir(folder))],1.])
            self.image_name_neg = [os.path.join(self.path_negtive, i) for i in os.listdir(self.path_negtive)]
            for fd in self.image_name_neg:
                fdlist = []
                findDepestFolder(fd, fdlist)
                for folder in fdlist:
                    self.imageAll.append([[os.path.join(folder, i) for i in sorted(os.listdir(folder))], 0.])
