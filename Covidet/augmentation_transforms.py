#==================================
'''
Transforms used in 3D blocks as policies
'''
...
MEANS = {
    ...
    'H':[0.,0.,0.],
    'eye':[0.1552138492848401, 0.2855536948533397,0.4463296922469498],
    'AddNoise':[0.4951994540168879,0.49413567928584506,0.4905329068393595]
}
STDS = {
    ...
    'H':[1./255.,1./255,1./255],
    'eye':[0.1476093989184158,0.21078306810335878,0.2917685697937044],
    'AddNoise':[0.08913075419725634,0.12143960210148051,0.11874566770061311]
}
PARAMETER_MAX = 10  # What is the max 'level' a transform could be predicted

def wrap3d(img,dataset):
    return img
def unwrap3d(img,dataset,image_size):
    return img
...
def pil_unwrap(pil_img, dataset, image_size):
    ...
    pic_array = 1.*pic_array.transpose((2,0,1))
    #pic_array = (pic_array - np.array(MEANS['AddNoise']).reshape(3,1,1))/np.array(STDS['AddNoise']).reshape(3,1,1)
    return pic_array
...
def flip3D1(block,level):
    return block[::-1,:,:]
def flip3D2(self,block,level):
    return block[:,::-1,:]
def flip3D3(self,block,level):
    return block[:,:,::-1]
def translation3D1(self,block,level):
    step = int_parameter(level, 3)
    if random.random() > 0.5:
        step = -step
    return block[16+step:48+step,16:48,:]
def translation3D2(self,block,level):
    step = int_parameter(level, 3)
    if random.random() > 0.5:
        step = -step
    return block[16:48, 16+step:48+step,:]
...
ALL_TRANSFORMS1 = [Flip3D1,Flip3D2,Flip3D3,Translation3D1,Translation3D2]
NAME_TO_TRANSFORM1 = {t.name: t for t in ALL_TRANSFORMS1}
TRANSFORM_NAMES1 = NAME_TO_TRANSFORM1.keys()
